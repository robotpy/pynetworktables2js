...
function loadCameraOnConnect(args) {
  ...
  function create_remote_uri(path) {
    ...
    if (args.proto) {
      ...
      if (
        ...
        !camera_url.startsWith("https://")
      ) {
        camera_url = "http://" + camera_url;
      }
    }

    if (args.port) camera_url = camera_url + ":" + args.port;
  ...
  function onSimulated() {
    ...
    var e = set_svg();
    e.append(
      $s("text").attr({ x: cx, y: cy, fill: "white" }).text("Simulated camera")
    );
  }
  ...
  function onSuccess() {
    ...
    console.log(
      dbg + "successfully connected to server, loading webcam at " + webcam_url
    );
  ...
  function tryConnect() {
    ...
    if (errors == 0)
      console.log(
        dbg + "Detecting webcam server via connection to " + detect_uri
      );
